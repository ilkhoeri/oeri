...
import { CommandIcon, CodeSquareIcon, FolderSearchIcon, HomeIcon } from "@/icons/*";
...
function CommandDataTypesSingleDemo() {
  return <Command store={dataSingleStore} container={null} forceMount actions={dataSingle} nothingFound="Oops..." searchProps={{ autoFocus: false }} />;
}

function CommandDataTypesGroupDemo() {
  return <Command store={dataGroupStore} container={null} forceMount actions={dataGroup} nothingFound="Nothing..." searchProps={{ autoFocus: false }} />;
}

function CommandConfiguratorDemo(props: CommandProps) {
  return <Command {...props} store={configuratorStore} container={null} forceMount actions={dataGroup} searchProps={{ autoFocus: false, leftSection: <FolderSearchIcon stroke={1.5} /> }} classNames={{ closeCommand: "not-sr-only flex" }} />;
}
<div className=""></div>;
const [firstStore, firstStoreState] = createCommand();
const [secondStore, secondStoreState] = createCommand();
