import { IconType } from "../../components/web/svg";
...
export function LoaderOrbit({
  ...
  styles,
  icon: Icon
}: LoaderBerdikarierProps) {
  return (
    <div
      className={[style.loader_root, classNames?.root].join(" ")}
      style={styles?.root}>
      <div
        className={[style.loader_wrap, classNames?.wrap].join(" ")}
        style={styles?.wrap}>
        <span
          ...
          style={styles?.inner}>
          <span
            className={[style.orbit, classNames?.orbit].join(" ")}
            style={styles?.orbit}
          />
          <span
            className={[style.orbit, classNames?.orbit].join(" ")}
        ...
        {Icon && (
          <Icon size={42} className={classNames?.icon} style={styles?.icon} />
        )}
      </div>
    </div>
  );
