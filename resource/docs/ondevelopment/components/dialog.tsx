...
const useDialogContext = (value?: string) => {
  ...
  const { isOpen, shouldRender, handleOverlayClick, toggle, side, ...rest } =
    ctx;
  return {
    isOpen: isOpen(value),
...
export const DialogProvider: React.FC<DialogProviderProps> = ({
  ...
  const [renderStates, setRenderStates] = React.useState<
  ...
  const shouldRender = (value: string | undefined) =>
    value ? !!renderStates[value] : false;
...
>(({ type = "button", className, id, ...props }, ref) => {
  ...
  return (
    <button
      ...
      onClick={ctx.toggle}
      className={twMerge(
        "group relative z-50 h-9 min-w-24 rounded-md bg-color px-2 text-center font-medium text-background",
        className
      )}
      {...props}
    />
...
export const DialogContent = React.forwardRef<
  ...
  React.ComponentPropsWithoutRef<"div"> & { value?: string; side?: Side }
