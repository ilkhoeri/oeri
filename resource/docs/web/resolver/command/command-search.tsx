...
import { factory, useProps, CompoundStylesApiProps, ElementProps } from "../factory";
...
export type CommandSearchFactory = ExtendsFactory<HTMLInputElement, CommandSearchProps>;
...
export const CommandSearch = factory<CommandSearchFactory>(function CommandSearch(props, ref) {
  // prettier-ignore
  ...
  const onClose = () => {
    ...
    commandActions.clearCommandState({ clearQuery: clearQueryOnClose }, ctx.store!);
  };
  ...
  return (
    <div {...ctx.getStyles("searchWrap", { classNames, styles })}>
      ...
      <input {...input} />
      <RightSection {...{ ...ctx.getStyles("closeCommand"), onClose, rightSection }} />
    </div>
  );
});

const LeftSection = React.forwardRef<HTMLLabelElement, React.ComponentProps<"label"> & { leftSection: React.ReactNode }>(function LeftSection({ leftSection, ...props }, ref) {
  return (
    leftSection ?? (
      <label {...{ ref, ...props }}>
        <svg fill="currentColor" stroke="none" strokeWidth="0" viewBox="0 0 48 48" xmlns="http://www.w3.org/2000/svg" className="mr-2 size-4 shrink-0 opacity-50">
          <path
            data-command="search"
...
>(function RightSection({ rightSection, type = "button", tabIndex = -1, onClick, onClose, ...props }, ref) {
  return (
    rightSection ?? (
      <button
        {...{
