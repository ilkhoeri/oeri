...
export type UnstyledButtonProps<Exclude extends string = never> = React.PropsWithoutRef<
  Omit<Omit<React.ComponentProps<"button">, "color" | "style"> & {
      asChild?: boolean;
      loading?: boolean;
...
export const UnstyledButton = React.forwardRef<HTMLButtonElement, UnstyledButtonProps>(
  (_props, ref) => {
    ...
    return <Btn {...{ ref, type, role, disabled: loading || disabled, ...props }}>{asChild ? enhancedChildren : <>{loadingComponent}{children}</>}</Btn>;
  }
);
...
export interface ButtonProps
  extends UnstyledButtonProps<"color">, cvxProps<typeof buttonVariants> {
  unstyled?: boolean;
}
