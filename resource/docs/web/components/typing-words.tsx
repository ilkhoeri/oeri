...
export function useTypingWords({ duration, placeholders }: UseTypingWords) {
  ...
  React.useEffect(() => {
    ...
    const animateTextTyping = (currentText: string[], remainingText: string[], element: HTMLElement, onAnimationEnd: () => void) => {
    ...
    const animateTextDeleting = (currentText: string[], element: HTMLElement, onAnimationEnd: () => void) => {
      ...
      animationId = setTimeout(
        ...
        getRandomDelayBetween(duration?.deleting || 100, duration?.deleting || 100)
      );
    };
...
export interface TypingWordsProps extends UseTypingWords, Omit<React.DetailedHTMLProps<React.HTMLAttributes<HTMLElement>, HTMLElement>, "style" | "children"> {
...
export const TypingWords = React.forwardRef<HTMLElement, TypingWordsProps>((_props, ref) => {
  const { el = "div", dir = "ltr", placeholders, duration, suppressHydrationWarning = true, withCursor, className, ...props } = _props;
