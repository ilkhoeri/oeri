...
const classes = cvx({
  variants: {
    selector: {
      header:
        "relative flex h-[calc(100dvh-var(--navbar))] w-full max-w-7xl flex-col items-center justify-avenly px-6 [--sz-circle:clamp(3rem,1rem+5dvw,4.5rem)] [--sz-rx:2.25rem] [--sz-ry:7.5rem] [transition:height_185ms_ease]",
      ...
      typingWords:
        "z-9 ml-[12dvw] h-[--sz] select-none bg-clip-text leading-none [--cursor-color:#6e5494] [--cursor-h:calc(var(--sz))] [background:--bg] [display:flex] [text-shadow:--t-sh]",
    ...
    <>
      <section id="header-section" className={merge(classes({ selector: "header" }), style.header_home)}>
          ...
          >
            {links.map((i, index) => (
              <Link
                ...
                href={i.url}
                target={i.target}
                data-link={sanitizedWord(i.title)}
                className={merge(classes({ selector: "links" }), [
                  { "[&_svg]:fill-current before:bg-[#6e5494] before:ring-[#6e5494]": i.title === "Repo" },
                  { "before:bg-color before:ring-color": i.title === "Docs" }
                ])}
              >
        ...
        <Transform
          el="s"
          data-linetop=""
          className={classes({ selector: "lineTop" })}
        ...
        >
          <Svg viewBox="0 0 218 179" stroke={3} className={classes({ selector: "svg" })}>
...
export function ThemesToggle({ size }: { size?: number | string }) {
  ...
  return (
    <Transform
      ...
      style={{ "--sz-circle": size ? rem(size) : undefined }}
    >
      <input
        checked={isDark}
