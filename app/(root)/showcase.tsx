  ...
  const tableData: DataTableProps = {
    ...
    head: [
      <Label key={"select-all"} htmlFor="select-all" aria-label="select-all" className="mt-1.5 h-[17px] cursor-pointer">
        <Input type="checkbox" id="select-all" name="select-all" className="bg-color [--checked-color:hsl(var(--color))] checked:bg-background" aria-label="Select All" checked={data.length > 0 && isAllSelected} indeterminate={isSomeSelected} onChange={handleSelectAll} />
        &nbsp;&nbsp;All
      </Label>,
      "Element name",
      "Symbol",
