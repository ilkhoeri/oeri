...
type RatingProps = StylesNames<"wrap" | "span" | "icon"> & {
...
export const Rating = React.forwardRef<HTMLDivElement, RatingProps>(function Rating({ rating = 0, totalStars = 5, onRatingChange, className, classNames, style, styles, unstyled, ...props }, ref) {
  ...
  return (
    <div
      {...{
        ref,
        className: cn({ "flex flex-row flex-nowrap items-center gap-1": !unstyled }, className, classNames?.wrap),
      ...
      {[...Array(totalStars)].map((_, index) => (
        <span
          ...
          {...{
            className: cn(
              {
                "cursor-pointer fill-[#666] [--sz-stars:1.25rem] data-[rating=active]:fill-[#ffa621]": !unstyled
              },
              classNames?.span
            ),
            style: styles?.span
        ...
        >
          <svg
            ...
            {...{
              className: cn(
                {
                  "size-[--sz-stars] max-h-[--sz-stars] min-h-[--sz-stars] min-w-[--sz-stars] max-w-[--sz-stars] [transition:fill_0.3s_ease]": !unstyled
                },
                classNames?.icon
              ),
              style: styles?.icon
