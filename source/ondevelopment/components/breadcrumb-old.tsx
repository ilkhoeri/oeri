...
const BreadcrumbList = React.forwardRef<HTMLOListElement, React.ComponentPropsWithoutRef<"ol"> & Unstyled>(({ unstyled, className, ...props }, ref) => (
  <ol
    ref={ref}
    className={cn(
      !unstyled && "flex w-max max-w-full flex-row flex-wrap items-center gap-1.5 overflow-auto break-words text-sm text-muted-foreground sm:gap-2.5",
      className
    )}
    {...props}
  />
...
const BreadcrumbItem = React.forwardRef<HTMLLIElement, React.ComponentPropsWithoutRef<"li"> & Unstyled>(({ unstyled, className, ...props }, ref) => (
  <li ref={ref} className={cn(!unstyled && "inline-flex items-center gap-1.5", className)} {...props} />
));
BreadcrumbItem.displayName = "BreadcrumbItem";

const BreadcrumbLink = React.forwardRef<HTMLAnchorElement, LinkProps & React.AnchorHTMLAttributes<HTMLAnchorElement> & Unstyled & { active?: boolean }>(
...
const BreadcrumbPage = React.forwardRef<HTMLSpanElement, React.ComponentPropsWithoutRef<"span"> & Unstyled>(({ unstyled, className, ...props }, ref) => (
  <span ref={ref} role="link" aria-disabled="true" aria-current="page" className={cn(!unstyled && "font-normal text-muted-foreground", className)} {...props} />
));
