...
export function getFilesWithPrefixWithExtension(segment: string[]) {
  const files = fs.readdirSync(
    `resource/docs_demo/${readdirPrefix("readdir", segment)}`
  );
  return files.filter(file =>
    file.startsWith(readdirPrefix("prefix", segment))
  );
}

export function getFilesWithPrefix(segment: string[]): string[] {
  const directoryPath = `resource/docs_demo/${readdirPrefix("readdir", segment)}`;
