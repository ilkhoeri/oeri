...
export function formatTitle(text: string | undefined): string {
  ...
  return text
    .toLowerCase() // Ubah semua huruf ke huruf kecil dulu
    .split(" ") // Pecah menjadi array kata
    .map((word, index) => {
      // Jika kata adalah kata hubung dan bukan kata pertama, biarkan huruf kecil
      if (conjunctions.includes(word) && index !== 0) {
        return word;
      }
