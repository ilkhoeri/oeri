...
export const useIdsContext = () => {
  const context = useContext(Context);
  if (!context) {
    throw new Error("useIdsContext must be used within an IdsProvider");
  }
  return context;
};

export const IdsProvider = ({ children }: { children: ReactNode }) => {
...
export const ChildWrapper = ({ children }: { children: React.ReactNode }) => {
  const { addIds, resetIds } = useIdsContext();
