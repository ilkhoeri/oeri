...
export function markdownText(text: string | undefined): string {
  if (!text) return "";
  // Replace ___ with <hr>
  ...
  text = text.replace(
    ...
    /```([^```]+)```/gs,
    (_, p1) =>
      `<div data-area="clipboard" class="clipboard-content notranslate position-relative overflow-auto"><pre class="notranslate"><code>${p1.trim()}</code></pre><span role="button" aria-label="Copy" class="clipboard-button" data-value="${stripHtml(
        p1
      )}" tabindex="0"><svg width="16" height="16" class="clipboard-icons size-4"><use class="icons-path" href="/assets/icons.svg#copy"></use></svg></span></div>`
  );
      ...
      if (textToCopy) {
        navigator.clipboard
          ...
          .then(() => {
            ...
            useElement.setAttribute("href", "/assets/icons.svg#check");
          })
          .catch(err => {
