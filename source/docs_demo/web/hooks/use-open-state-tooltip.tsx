...
export function Demo() {
  ...
  return (
    <>
    <__docs_demo.area>
    <Button ref={state.refs.trigger} {...state.attr}>
      Tooltip
    </Button>
    </__docs_demo.area>

    <state.Portal render={state.render}>
      <p role="tooltip" ref={state.refs.content} {...state.attrStyles("content")} className={tooltip({ side: side as `${DataSide}` })}>
        useOpenState allows use as a hook tooltip
      </p>
    </state.Portal>
    <__docs_demo.controls><__docs_demo.setSelect label="Side" values={Object.values(DataSide)} str={side} setStr={setSide} /></__docs_demo.controls>
    </>
  );
}
